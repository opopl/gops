...
#
##########
#   NOTE: This script is used to install OPTIM + CHARMM35. If you want to install
#         OPTIM + CHARMM31 then you should use the original build.csh script.
#   Example:  ./build35.csh coptim pgi amd noopt
##########
#
# set parameters
...
foreach opt ( $arguments )
  ...
  if ( $readnext != 1 ) then
    set readnext = 1
  else if ( $opt == 'optsrc' || $opt == 'c35src' ) then
    set readnext = 0
...
if ( $target == optim ) then
  ...
  set CTYPE = C35
  set c35src = $c35src
  echo "build,csh> c35src reset to " $c35src
  ...
  set LIBDIR35 = $c35src"/lib/gnu"
  set SRC35 = "charmm_main.src energy.src"
...
if ( $target == coptim ) then # need to compile CHARMM first
   if (! -e $c35src ) then
     echo "build.csh> ERROR: c35 source directory $c35src does not exist!"
     exit
   else
     echo "build.csh> c35src = $c35src"
   endif
...
#   echo "build.csh> cd ${c35src} && ${c35src}/$command && cd $optsrc"
   echo "build.csh> cd ${c35src} && ./$command && cd $optsrc"
   echo
#   cd ${c35src} && ${c35src}/$command && cd $optsrc
   cd ${c35src} && ./$command && cd $optsrc
...
$make $quiet $EXE FC=$FC NOOPT="$NOOPT" SEARCH_PATH="$SEARCH_PATH $JBMODS" FFLAGS="$FFLAGS" FREEFORMAT_FLAG="$FREEFORMAT_FLAG" EXTRA_FLAGS="$EXTRA_FLAGS" EXTRA_CFLAGS="$EXTRA_CFLAGS" $target="$EXE" LIBDIR35="$LIBDIR35" \
BLAS_EXCLUDE_LIST="$BLAS_EXCLUDE_LIST" CTYPE="$CTYPE" PREFLX="$PREFLX" FCMDIR="$FCMDIR" PREFDIR="$PREFDIR" SRC35="$SRC35" \
C35SRC="$c35src" SWITCH="$SWITCH" >& make.out && set success = 1
