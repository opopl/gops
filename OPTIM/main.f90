!   OPTIM: A program for optimizing geometries and calculating reaction 
...
!                                                                       
      ...
      USE PORFUNCS 
      USE OPTIMHEADER 
      USE KEY, ONLY: FILTHSTR,SEQ,NUMGLY,TARFL,CASTEPJOB,CP2KJOB,ONETEPJ 
      USE MODAMBER9, ONLY: AMBERSTR,AMBERSTR1,INPCRD,ATMASS1 
                                                                        
      IMPLICIT NONE 
                                                                        
      INTEGER ITEM, NITEMS, LOC, LINE, NCR, NERROR, IR, LAST, J1 
      LOGICAL VARIABLES,CASTEP,ONETEP,CP2K,DFTP,CPMD,END,CAT,SKIPBL,CLEA 
...
!                                                                       
!  The function iargc returns the argument count                        
!  The statement call getarg( k , arg ) gets the  kth  command-         
!  line argument and puts it into arg.                                  
!  The 0th argument is the command name.                                
!                                                                       
!  This provides a new way to filthify OPTIM.                           
!                                                                       
                            ! SAT: subroutine interface to iargc        
      CALL iargc_subr(NARG) 
      FILTH=0 
      FILTH2=0 
      IF (NARG.GT.0) THEN 
         CALL GETARG(1,ARGSTRING) 
!                                                                       
!  Both methods to convert the character to an integer seem to work.    
!                                                                       
!  It would be better to use ARGSTRING as a string, so that non-numerica
!  extensions would work. This would require corresponding changes to Fi
!  We could also remove the odata.read file if Filthy_Phyllis were chang
!  with fork and wait!                                                  
!                                                                       
!        FILTH2=ATOI(ARGSTRING)                                         
         READ(ARGSTRING,*) FILTH2 
         WRITE(*,'(A,I6,A)') ' Command line argument read, using extensi 
         FILTH=FILTH2 
      ELSE 
         ARGSTRING='' 
      ENDIF 
  ...
  190 CALL INPUT(END) 
      ...
      IF (WORD.EQ.'    ' .OR.WORD.EQ.'NOTE'.OR.WORD.EQ.'COMMENT'.OR. WOR 
         GOTO 190 
  ...
  310    READ (UNIT=9,IOSTAT=ios,FMT='(A3)') check                      
         IF (ios.LT.0) THEN                                             
           ...
           STOP                                                         
         ENDIF                                                          
         IF (check.EQ.'end' .OR. check.EQ.'END' .OR. check.EQ.'End') THE
            CLOSE(9)                                                    
            GOTO 400                                                    
         ENDIF                                                          
         NATOMS=NATOMS+1                                                
...
!         NATOMS=0                                                      
       ...
       IF(NITEMS<3) then                                                
         CALL READA(amberstr)                                           
          IF (FILTH2.NE.0) THEN                                         
            WRITE(OTEMP,*) FILTH2                                       
            write(ostring,'(A)') trim(adjustl(amberstr))//'.'//trim(adju
            WRITE(*,*) 'ostring=', ostring                              
          ELSE                                                          
            write(ostring,'(A)') trim(adjustl(amberstr))                
          END IF                                                        
          WRITE(*,'(A)') ' getparams> input coordinates for AMBER9 syste
     ...
     &                         'type: ', trim(adjustl(amberstr1))       
          IF(trim(adjustl(amberstr1)).EQ.'inpcrd') then                 
           inpcrd=amberstr                                              
           WRITE(*,'(A)') ' getparams> reading AMBER inpcrd coordinate f
          ELSE                                                          
           WRITE(*,'(A)') ' getparams> ERROR - no other types defined cu
          END IF                                                        
           call amberinterface(natoms,2,inpcrd,6)                       
       END IF                                                           
        GOTO 400                                                        
...
! DAE We are going to assume that there is a charmm format file in the d
! line of it will tell us the number of atoms. In 99% of old (OPTIM<3) r
! were more flexible in that any filename could be specified in the CHAR
         ...
         IF (ios /= 0) THEN                                             
            ...
            WRITE(*,'(2A)') 'Thanks to our new dynamic memory allocation
     &    '"input.crd" for CHARMM to find out the number of atoms. Feel 
            STOP                                                        
         ENDIF                                                          
         ENDIF                                                          
         do                                                             
...
! DAE We also need to find out what MAXAIM is in CHARMM, and set MXATMS 
! are passed between the two can be declared correctly. MXATMS is now st
...
! jmc We are going to assume that there is a coords file. The first line
         ...
         IF (ios /= 0) THEN                                             
            WRITE(*,'(2A)') 'Thanks to our new dynamic memory allocation
     &    ' for OPTIM3 to find out the number of atoms for UNRES. Please
            STOP                                                        
         ENDIF                                                          
         READ(9,*) NATOMS                                               
         CLOSE(9)                                                       
...
!           WRITE(*,'(A21)') DSTRING1                                   
            IF (DSTRING1(1:16).EQ.'%BLOCK POSITIONS') THEN              
               DO                                                       
                  ...
                  CALL UPPERCASE(DSTRING1)                              
!                 WRITE(*,*) DSTRING1                                   
                  IF (DSTRING1(1:19).EQ.'%ENDBLOCK POSITIONS') EXIT coor
                  NATOMS=NATOMS+1                                       
               ENDDO                                                    
            ENDIF                                                       
         ENDDO coordsloop                                               
...
!                 WRITE(*,*) DSTRING1                                   
      ...
      ELSE IF (CPMD) THEN                                               
         FNAME=SYS(1:LSYS)                                              
         WRITE(*,'(A,A)') 'getparams> Reading coordinates from file ',FN
         OPEN(UNIT=7,FILE=FNAME,STATUS='OLD')                           
         NATOMS=0                                                       
   11    READ(7,'(A)') FNAME                                            
  ...
  400 CLOSE(5)                                                          
      ...
      WRITE(*,'(A,I6)') ' getparams> Number of atoms (or variables)  det
                                                                        
      CALL OPTIM(FILTH,FILTH2,ARGSTRING)                                
                                                                        
      STOP                                                              
                           ! finished counting atoms, go to the end of t
                                         ! SAT This is the goddamn CHARM
      END                                           
