...
c format 3  -  start with random structure and expand quartic polynomial
      ...
      use amhglobals,  only: AMHmaxsiz,maxtab,maxcnt,maxcrd,i_bias_native_b,
...
c        write(6,*)'Q ',Q
     ...
     * + 6*Q_weight_b*(Q0_b)**2*(Q)**2 + (-4)*Q_weight_b*Q0_b**3*(Q)**1
        ...
        dV_dQ=real(n_Qbias_b)*Q_weight_b*(Q-Q0_b)**(n_Qbias_b-1)
      elseif (i_Q_format_b.eq.3.and.(Q-Q0_b.gt.Q_clip_b)) then
        dV_dQ=real(n_Qbias_b)*Q_weight_b*(Q_clip_b)**(n_Qbias_b-1)
