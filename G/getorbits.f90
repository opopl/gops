...
ISTART=1
DO J=1,NORBIT
   IF (LDEBUG) WRITE(LFH, '(A,I6,A,I6,A,F15.5)') 'orbit ',J,' size ',ORBSIZE(J),' distance ',ORBDIST(J)
   IF (LDEBUG) WRITE(LFH, '(A,I6)') 'members of orbit ',J
   IF (LDEBUG) WRITE(LFH, '(12I5)') (NINDEX(J1),J1=ISTART,ISTART+ORBSIZE(J)-1)
   IF (LDEBUG) WRITE(LFH, '(A)') 'distances:'
   IF (LDEBUG) WRITE(LFH, '(6G20.10)') (CMDIST(J1),J1=ISTART,ISTART+ORBSIZE(J)-1)
   ...
   ISTART=ISTART+ORBSIZE(J)
ENDDO
IF (LDEBUG) WRITE(LFH,'(A,I6,A,I6)')  'largest orbit has ',LARGESIZE,' members starting from sorted position ',LARGES
