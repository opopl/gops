...
INTEGER J1, J2, ICPU, PID(NCPU), MINS(NCPU), MINF(NCPU), STATUS, NCYCLES, NMINOLD, NTSOLD, ISTAT, NEWJOB, PIDDONE, NREJ, NINITIAL
...
! {{{
   ...
   IF (DIJPAIRT) THEN
      CALL GETDPAIR(NAVAIL,NUSED,MINS(ICPU),MINF(ICPU),SPOINTS,FPOINTS)
   ELSEIF (CONNECTREGIONT) THEN
      CALL GETRPAIR(NAVAIL,NUSED,MINS(ICPU),MINF(ICPU),SPOINTS,FPOINTS)
!  ELSEIF (PTAUT) THEN
!     CALL GETPPAIR(NAVAIL,NUSED,MINS(ICPU),MINF(ICPU),SPOINTS,FPOINTS)
   ELSEIF (SHORTCUTT) THEN
      CALL GETSPAIR(NAVAIL,NUSED,MINS(ICPU),MINF(ICPU),SPOINTS,FPOINTS)
   ELSEIF (UNTRAPT) THEN
      CALL GETUPAIR(NAVAIL,NUSED,MINS(ICPU),MINF(ICPU),SPOINTS,FPOINTS)
   ELSEIF (FREEPAIRT) THEN
      CALL GETFREEPAIR(NAVAIL,NUSED,MINS(ICPU),MINF(ICPU),SPOINTS,FPOINTS)
   ELSEIF (USEPAIRST) THEN
      CALL GETNCONN ! must call this first to set NCONNMAX - used for declarations in GETUSEPAIR
      CALL GETUSEPAIR(NAVAIL,NUSED,MINS(ICPU),MINF(ICPU),SPOINTS,FPOINTS)
   ELSE
      CALL GETPAIR(NAVAIL,NUSED,MINS(ICPU),MINF(ICPU),SPOINTS,FPOINTS)
   ENDIF
   CALL CONNECTODATA(ICPU,SPOINTS,FPOINTS)
   CALL FLUSH(6,ISTAT) ! the child process may duplicate output without this line!
   CALL FORK_SUBR(PID(ICPU)) ! PID is zero in the child, non-zero in the parent
   IF (DEBUG.AND.(PID(ICPU).NE.0)) WRITE(*,'(A,I8)') 'cycle2> forked connect run process id=',PID(ICPU)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!  IF (PID(ICPU).NE.0) WRITE(*,'(A,I8)') 'cycle2> forked connect run process id=',PID(ICPU)
!  IF (PID(ICPU).EQ.0) THEN
!     WRITE(*,'(A,I8)') 'cycle2> I am the child! PID=',PID(ICPU)
!     CALL GETPID_SUBR(CHILDPID)
!     PRINT *,'CHILDPID=',CHILDPID
!  ENDIF
!  PRINT '(A,2I8)','cycle2> ICPU,PID=',ICPU,PID(ICPU)
