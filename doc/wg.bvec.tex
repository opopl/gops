...
\begin{align}
  ...
  \nn\DPD(i)&=(\v{b}_i,\v{b}_{i+1})=(\rr_i-\rr_{i+1},\rr_{i+1}-\rr_{i+2})\\
...
\begin{align*}
 \DPD(i,1),&\quad\DPD(i-1,1),\quad\DPD(i),\quad\DPD(i-1),\quad\DPD(i-2),\\ 
...
\begin{table}
  ...
  \begin{tabular}{ccc}
   $i$ 		& Number of DPs		&	DPs \\ 
   $1$		& 3			&	$\DPD(1,1),\DPD(1),\DPD(1,3)$ \\
   $2$		& 6			&	$\DPD(2,1),\DPD(1,1),\DPD(2),\DPD(1),\DPD(2,3),\DPD(1,3)$ \\
   $N-1$	& 6			&	$\DPD(N-1,1),\DPD(N-2,1)$
						$\DPD(N-2),\DPD(N-3)$,
						$\DPD(N-3,3),\DPD(N-4,3)$	\\	
   $N$		& 3			&	$\DPD(N-1,1),\DPD(N-2),\DPD(N-3,3)$ \\
  \end{tabular}
\end{table}
...
\begin{align}
  ...
  \frac{\partial\DPD(i)}{\partial \rr_i}&=2(\rr_i-\rr_{i+1})=2\bb_i\\
\end{align}
