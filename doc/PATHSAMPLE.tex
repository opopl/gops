...
\title{{\tt PATHSAMPLE.2.1} User Guide}
...
% <kwd>
\begin{itemize}
\item {\it ADDPATH afile \/}: reads pathway data from file {\tt afile}. If this 
...
\item {\it ANGLEAXIS \/}: specifies angle-axis coordinates for rigid bodies.
...
\item {\it CHARMM n\/}: specifies that the CHARMM potential is used in the {\tt OPTIM} runs.
...
\item {\it CPUS ncpu}: specifies the number of cpu's available on an SMP machine. If used on
...
\item {\it DIJINITCONT exp or n\/}: continue an initial Dijkstra connection 
...
\item {\it DIJINITSTART exp or n\/}: perform an initial Dijkstra connection run 
...
\item {\it DIRECTION a\/}: the direction (`AB' or `BA') determines the sense in which KMC trajectories
...
\item {\it JOBSPERNODE jpn\/}: specifies the number of jobs to run on each node for a
...
\item {\it KSHORTESTPATHS npaths nconnmin\/}: calculate the {\it npaths\/} paths
...
{\it SGT}, {\it DGT} and {\it SDGT}. The connection condition is evaluated recursively.

\item {\it NGT nconnmin disconnectsources density size\/}: specifies that the new
...
\item {\it PERMDIST\/}: specifies that all distance metrics should include minimisation
...
\item {\it PFOLD n1 n2 x\/}: provides control parameters for the on-the-fly calculation
...
\item {\it PULL \/}: add this keywork if the system has a non-zero static force 
...
\item {\it STARTFROMPATH afile n1 n2 \/}: reads an initial path from file {\tt afile}. If this 
...
\item {\it UNTRAP einc thresh\/}: try connecting minima to eliminate traps.
