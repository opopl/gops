...
USE QMODULE
USE PORFUNCS
USE OUTPUT, ONLY : WRITE_COORDS, WRITE_MARKOV_COORDS

IMPLICIT NONE

! subroutine {{{
INTEGER NDONE, JBEST(NPAR), JP
...
!  WRITE(MYUNIT2, '(3F25.15)') COORDSO(1:3*NATOMS,JP)
  ...
  WRITE(MYUNIT2, '(4F25.15)') STEP,ASTEP,OSTEP,TEMP
...
! subroutine   {{{
INTEGER NDONE, JBEST(NPAR), JP
DOUBLE PRECISION :: EBEST(NPAR)
DOUBLE PRECISION  BESTCOORDS(3*NATOMS,NPAR)
! }}}
! local  {{{
INTEGER ::  J1, OLDSAVE, MYUNIT2
DOUBLE PRECISION :: DUMMYARRAY(3*NATOMS)
...
! step and temperature information
   READ(MYUNIT2,*) DUMMYS
   READ(MYUNIT2,*) STEP,ASTEP,OSTEP,TEMP
! best OLDSAVE minima
  READ(MYUNIT2,*) DUMMYS
