...
USE COMMON,ONLY : NCONN, NGTSWITCH, NGTSIZE
USE PORFUNCS
USE NGTMEM
IMPLICIT NONE
...
DO MIN1=NMIN,NMINA+NMINB+1,-1
   ...
   IF ((MINCONN*1.0D0)/(MIN1*1.0D0)**2.GE.NGTSWITCH) THEN
      IF (MIN1.LE.NGTSIZE) THEN 
         PRINT '(A,F12.3,A)','NGTremovei> about to switch to dense scheme and allocate storage of ',MIN1*MIN1*8.0D-6,' Mb'
         PRINT '(A,F12.6,A,I6)','NGTremovei> density=',(MINCONN*1.0D0)/(MIN1*MIN1*1.0D0),' remaining minima=',MIN1
         CALL FLUSH(6,ISTAT)
