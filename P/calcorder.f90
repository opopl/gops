...
! loc
INTEGER J1, J2, J3, NDUMMY
DOUBLE PRECISION LOCALPOINTS(3*NATOMS), THISORDER, DIST, CMX, CMY, CMZ, RADG, CHINATIVE, CHINON
...
!
  ...
  &                 (LOCALPOINTS(3*(J1-1)+3)-LOCALPOINTS(3*(J2-1)+3))**2 )
   ...
   IF (THISORDER.GT.CHINATIVE) THEN
      WRITE(1,'(I8,2G20.10)') J3, THISORDER, RADG
   ELSEIF ((THISORDER.LT.CHINON).AND.(RADG.GT.2.5D0).AND.(RADG.LT.4.5D0)) THEN
