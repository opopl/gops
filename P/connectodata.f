...
C
      ...
      DOUBLE PRECISION DISTANCE, LOCALPOINTS1(NR), LOCALPOINTS2(NR), SAVEPOINTS(NR), RMAT(3,3), DIST2
...
!        CLOSE(1)
     ...
     1                               LOCALPOINTS1(3*(J2-1)+3),J2=1,NATOMS)
      ...
      ELSE IF (AMHT) THEN
         ...
         OPEN(2,FILE=TRIM(ADJUSTL(FPOO)),STATUS='UNKNOWN')
         WRITE(2,'(3G25.15)') LOCALPOINTS1(1:NR)
         CLOSE(2)

         FPOO='finish.'//TRIM(ADJUSTL(CONNSTR))
         OPEN(1,FILE=TRIM(ADJUSTL(FPOO)),STATUS='UNKNOWN')
         WRITE(1,'(3G25.15)') LOCALPOINTS2(1:NR)
         CLOSE(1)
