...
!
      ...
      DOUBLE PRECISION LOCALPOINTS(NR),NATIVEPOINTS(NR),HEL,DIST, &
     & HELRMSD,RMSD
      CHARACTER*10 STRING
      LOGICAL DEBUG
     ...
     & FORM='UNFORMATTED',STATUS='UNKNOWN',RECL=8*NR)
      ...
      DO J1=1,NMIN
        ...
        HELRMSD=0.0D0
        CNT=0	
        READ(10,REC=J1) (LOCALPOINTS(J2),J2=1,NR)
        DO J2=63,187
     ...
     &  (LOCALPOINTS(3*(127-1)+3)-LOCALPOINTS(3*(157-1)+3))**2)
        ...
        IF (HEL.LE.0.2D0.AND.HELRMSD.GE.7.5D0) THEN
          WRITE(91,'(a10,3f15.5)')STRING,HEL,HELRMSD,RMSD
        ELSEIF ((HEL.EQ.1.0D0).AND.(HELRMSD.LT.1.5D0).AND. &
     & (RMSD.LT.2.5D0)) THEN 
        WRITE(92,'(a10,3f15.5)') STRING,HEL,HELRMSD, RMSD
        ENDIF
