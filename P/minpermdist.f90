...
USE COMMONS,ONLY : NPERMGROUP, NPERMSIZE, PERMGROUP, NSETS, SETS, GEOMDIFFTOL, AMBERT, NFREEZE, CHARMMT, RBAAT, PULLT, &
  &               ANGLEAXIS, PERMISOMER, PERMDIST, ZSYM, INTCONSTRAINTT, INTLJT
...
DOUBLE PRECISION DIST2, COORDSA(NR), COORDSB(NR), DISTANCE, DUMMYA(NR), DUMMYB(NR), DUMMY(NR)
DOUBLE PRECISION BOXLX,BOXLY,BOXLZ,WORSTRAD,RMAT(3,3),ENERGY, VNEW(NR), DX, DY, DZ, RMS, DBEST, XBEST(NR)
...
DOUBLE PRECISION PDUMMYA(NR), PDUMMYB(NR), LDISTANCE, DUMMYC(NR), XDUMMY
INTEGER NEWPERM(NATOMS), ALLPERM(NATOMS), SAVEPERM(NATOMS), NMAXINT, NMININT
DOUBLE PRECISION CONSTRAINTE, XYZLOCAL(6*NATOMS), LMINCOORDS(2,NR)

! }}}

IF (USEINT) THEN
  ! {{{
   LMINCOORDS(1,1:NR)=COORDSB(1:NR)
   LMINCOORDS(2,1:NR)=COORDSA(1:NR)
   XYZLOCAL(1:NR)=LMINCOORDS(1,1:NR)
   XYZLOCAL(NR+1:6*NATOMS)=LMINCOORDS(2,1:NR)
...
!  PRINT '(3F20.10)',DUMMYA(1:NR)
!  PRINT '(A)','DUMMYB before NEWMINDIST:'
!  PRINT '(3F20.10)',DUMMYB(1:NR)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!
  ...
  &                         ' should be ',SQRT(DISTANCE)
      ...
      PRINT '(3F20.10)',XBEST(1:NR)
      PRINT '(A)','COORDSB:'
      PRINT '(3F20.10)',COORDSB(1:NR)
   ENDIF
