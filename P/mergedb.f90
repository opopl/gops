...
DOUBLE PRECISION LOCALPOINTS(NR), NEWEMIN, NEWETS
DOUBLE PRECISION NEWFVIBMIN, NEWFVIBTS, NEWPOINTSMIN(NR), NEWNEGEIG, &
  &  NEWPOINTSTS(NR), NEWIXMIN,  NEWIYMIN, NEWIZMIN, &
  &  NEWIXTS,  NEWIYTS, NEWIZTS, DISTANCE, DIST2, LOCALPOINTS2(NR), RMAT(3,3)
...
OPEN(UNIT=2,FILE=TRIM(ADJUSTL(FNAME)),ACCESS='DIRECT',FORM='UNFORMATTED',STATUS='OLD',RECL=8*NR)
NEWMIN=0
MINDB=0
...
!
   READ(2,REC=MINDB) (NEWPOINTSMIN(J2),J2=1,NR)  
...
!  DO J2=1,NMIN
  ...
  &                       RMAT,.FALSE.)
         ...
         IF ((ABS(NEWEMIN-EMIN(J2)).LT.1.0D-9).AND.(DISTANCE.GT.GEOMDIFFTOL)) THEN
            ...
            PRINT '(A)','LOCALPOINTS:'
            PRINT '(3F20.10)',LOCALPOINTS(1:NR)
            PRINT '(A)','LOCALPOINTS2:'
            PRINT '(3F20.10)',LOCALPOINTS2(1:NR)
            STOP !!! DJW
         ENDIF
...
!
  ...
  &                ' writing parameters to file min.data and points to points.min'
   ...
   WRITE(UMIN,REC=INDEX) (NEWPOINTSMIN(J2),J2=1,NR)
...
!  Read in points and check for agreement with moments of inertia as in setup
!
   READ(2,REC=TSDB) (NEWPOINTSTS(J2),J2=1,NR)  
...
!
   ...
   WRITE(UTS,REC=INDEX) (NEWPOINTSTS(J2),J2=1,NR)
