...
MODULE GRAPHTRANSFORMATIONMODULE
     ...
     SUBROUTINE SPARSEGRAPHTRANSFORMATION
          USE COMMONS, ONLY:DEBUG,SWITCH=>GT2SWITCH,RSWITCH=>GT2RSWITCH,DisconnectSources=>GT2DisconnectSources
          ...
          DO
               ...
               IF (ASSOCIATED(Z)) THEN
                    ...
                    IF (N==0) THEN
                         ...
                         if (DisconnectSources) then
                              CALL DenseGraphTransformation
                         else
                              print *, 'Output> Disconnection of sources was explicitly switched off'
                              call SGTOutput
                         endif
                         EXIT
     ...
     subroutine SGTOutput ! SAT, Fri Mar 16 00:33:07 GMT 2007
          use DataModule, only: G,Nnodes,S1
          implicit none
          integer :: i
          ...
          do i=1,Nnodes
               if (G(I)%P%T=='s') then
                    WRITE(S1,'(i20)') i
                    PRINT *, 'Output> Source '//trim(adjustl(s1))//':'
                    PRINT *, 'Output>    Tau =',G(i)%p%tau
                    TAUTOT=TAUTOT+G(I)%P%PROB*G(i)%p%tau
               endif
          enddo
          PRINT *,'Output> Tau total =',TauTot
     end subroutine SGTOutput
     SUBROUTINE SGDETACHNODE(C)
     USE COMMONS, ONLY:ALTPBB=>GT2ALTPBB,RESCALE=>GT2RESCALE
     ...
     SUBROUTINE DenseGraphTransformation
          USE DATAMODULE,ONLY: NUNPROCESSEDNODES,G,P,TAU
          USE COMMONS, ONLY:DEBUG,DisconnectSources=>GT2DisconnectSources
          IMPLICIT NONE
...
!         CALL PRINTMATRIX(SIZE(P,1),SIZE(P,2),P) ! DJW
          CALL CPU_TIME(TS)
          PRINT *, 'CGT> Start'
...
!         CALL PRINTMATRIX(SIZE(P,1),SIZE(P,2),P) ! DJW
          ...
          IF (DEBUG.AND.NUNPROCESSEDNODES<9) CALL PRINTMATRIX(SIZE(P,1),SIZE(P,2),P)
          CALL DGTOutput(IFIRST,SFIRST)
          DEALLOCATE(P,TAU)
     END SUBROUTINE DenseGraphTransformation
     SUBROUTINE CHANGEDS(IFIRST,SFIRST,SOURCES)
          ...
          ENDDO
          DEALLOCATE(GTMP)
          PRINT '(A,F20.1,A)','changeds> about to try and allocate ',1.0D0*NUNPROCESSEDNODES*(NUNPROCESSEDNODES-IFIRST), &
  &                        ' real*8 elements'
     ...
     SUBROUTINE DGTOutput(IFIRST,SFIRST)
          USE DATAMODULE,ONLY:G,P,TAU,S1,S2,NUNPROCESSEDNODES
          USE COMMONS, ONLY:DEBUG,DIRECTION,DisconnectSources=>GT2DisconnectSources
          IMPLICIT NONE
          INTEGER,INTENT(IN) :: IFIRST,SFIRST
          INTEGER :: I,J
          DOUBLE PRECISION :: TAUTOT, KTOT, Psum
          TAUTOT=0.D0
          KTOT=0.D0
          ! Semen: Wed Mar 14 17:37:02 GMT 2007
          if (.not.DisconnectSources) then
               print *, 'DGTOutput> Disconnection of sources was explicitly switched off'
          endif
          Psum=1.0D0
          DO I=SFIRST,NUNPROCESSEDNODES
               WRITE(S1,'(i20)') i
               ...
               ! SAT Thu Jan 11 00:25:45 GMT 2007
               if (tau(I)==0.0d0) then
                    print *, 'Output> Escape time is zero for source',I
               else
                    KTOT=KTOT+G(I)%P%PROB*Psum/TAU(I)
               endif
               IF (DEBUG) THEN
                    DO J=1,IFIRST-1
               ...
               ENDIF
!
! If all other sources have been disconnected then PSUM should be unity!
!
               PRINT *, 'Output>    P sum =',sum(P(1:sfirst-1,i))
          ENDDO
          PRINT *,'Output> Tau total =',TauTot
          IF (DIRECTION.EQ.'AB') PRINT '(1x,A,G20.10)','Output> rate constant k(A<-B) =',KTot
          IF (DIRECTION.EQ.'BA') PRINT '(1x,A,G20.10)','Output> rate constant k(B<-A) =',KTot
     END SUBROUTINE DGTOutput
     SUBROUTINE PRINTMATRIX(NROW,NCOL,P,NROWSTART)
          ...
          CHARACTER(LEN=100) :: F
          WRITE(F,'(i20)') ncol
          F='(1x,1a,'//trim(adjustl(f))//'g12.5,1x,1a)'
     ...
     SUBROUTINE DISCONNECT(A,B,IFIRST,SFIRST)
          ...
          PBA=P(B,A)
          D=1.0D0-PAB*PBA
          IF (D==0.0D0) THEN ! SAT Thu Jan 11 00:25:45 GMT 2007
               PRINT *
               PRINT *, "Disconnect> Time for escape from sources subgraph is infinite"
               PRINT *, "Disconnect> This is because (",A,B,") is a subgraph with infinite escape time"
               PRINT *, "Disconnect> You might be alright if you exclude sources",A,B,"from consideration"
               PRINT *, "Disconnect> This will only work if (",A,B,") is (effectively) disconnected subgraph"
               PRINT *, "Disconnect> From this point onwards this piece of software will output *GARBAGE* only"
               ! this is an example of how you might check that (A,B) is a disconnected subgraph
               !TMP=0.0D0
               !DO I=SFIRST,NUNPROCESSEDNODES
               !     TMP=TMP+P(A,I)
               !     TMP=TMP+P(B,I)
               !ENDDO
               !PRINT *, TMP
               ! if tmp/=0 the probability of visiting is non-zero and hence (A,B) is not a disconnected subgraph
          ELSE
               TMP=TAU(A)
