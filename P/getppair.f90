...
USE COMMONS, ONLY: UMIN, NATOMS, DMIN1, DMIN2, NATTEMPT, NCPU, MINSEP, BULKT, TWOD, ZSYM, DEBUG, BESTPATHLENGTH, ETS, &
  ...
  &               NCONNMIN, TSTHRESH
USE COMMONS, ONLY: NMIN, NTS, LOCATIONA, LOCATIONB, TOPPOINTER, POINTERM, POINTERP, NCONNMAX, NMINA, NMINB
USE COMMONS, ONLY: TEMPERATURE, REGROUPRATET, REGROUPPET, MINGROUP, MAXBARRIER
USE COMMONS, ONLY: NR
...
DOUBLE PRECISION,DIMENSION(NR) :: SPOINTS, FPOINTS(NR)
! loc 


INTEGER PAIRSTODO, J1, NSTEPS, J2, J3, J4, N1, N2, NLEFT, NUNCONA, NUNCONB, M1, M2, NCYCLE, DMIN, DMAX
!INTEGER NMINSAVE
INTEGER MINMAP(NMIN)
INTEGER, ALLOCATABLE :: MINLIST(:), POSITION(:)
INTEGER NCOL(NMIN), NDISTA(NMIN), NDISTB(NMIN)
INTEGER MIN1, MIN2, NDEAD
DOUBLE PRECISION, ALLOCATABLE :: DISTLIST(:), BHEIGHT(:)
DOUBLE PRECISION DISTANCE, RMAT(3,3), DIST2, PTIMESTAU(NMIN), LKSUM(NMIN), DINDEX(NMIN), &
...
!       DO J1=1,NSTEPS
!          READ(UMIN,REC=MINLIST(J1)) (SPOINTS(J2),J2=1,NR)
...
!                READ(UMIN,REC=MINLIST(J2)) (FPOINTS(J3),J3=1,NR)
