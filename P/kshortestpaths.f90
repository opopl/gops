...
DOUBLE PRECISION DMATMC(NTS,2), DUMMY, PFTOTALSTART, HUGESAVE, LOCALPOINTS(NR), KSUM(NMIN)
...
OPEN(UNIT=4,FILE='points.min.fastest.all',ACCESS='DIRECT',FORM='UNFORMATTED',STATUS='UNKNOWN',RECL=8*NR)
COUNTER = 0
DO J5=1,NMIN
   IF (ADDMIN(J5)) THEN
      COUNTER = COUNTER + 1
      WRITE(2,'(2F20.10,I6,3F20.10)') EMIN(J5), FVIBMIN(J5), HORDERMIN(J5), IXMIN(J5), IYMIN(J5), IZMIN(J5)
      READ(UMIN,REC=J5) (LOCALPOINTS(J2),J2=1,NR)
      WRITE(4,REC=COUNTER) (LOCALPOINTS(J2),J2=1,NR)
      INDEX_NEW_MIN(J5) = COUNTER
   ENDIF
ENDDO
CLOSE(2); CLOSE(4)
OPEN(UNIT=3,FILE='ts.data.fastest.all',STATUS='UNKNOWN')
OPEN(UNIT=5,FILE='points.ts.fastest.all',ACCESS='DIRECT',FORM='UNFORMATTED',STATUS='UNKNOWN',RECL=8*NR)
COUNTER = 0
DO J5=1,NTS
   IF (ADDTS(J5)) THEN
      COUNTER = COUNTER + 1
  ...
  &                                        IXTS(J5),IYTS(J5),IZTS(J5)
      ...
      IF (.NOT.DUMMYTST) THEN
         READ(UTS,REC=J5) (LOCALPOINTS(J2),J2=1,NR)
         WRITE(5,REC=COUNTER) (LOCALPOINTS(J2),J2=1,NR)
      ENDIF
   ENDIF
ENDDO
CLOSE(3); CLOSE(5)
...
DOUBLE PRECISION ALIGNPOINTS(NR), DIST2, DISTANCES, RMAT(3,3), LOCALPOINTS(NR), DUMMY
...
&            1.00,0.00,SEGID(J2)
         ...
         IF (J1.EQ.NSTEPS) THEN
            READ(UMIN,REC=PARENT(J5)) (LOCALPOINTS(J2),J2=1,NR)
            CALL MINPERMDIST(ALIGNPOINTS,LOCALPOINTS,NATOMS,DEBUG,BOXLX,BOXLY,BOXLZ,BULKT,TWOD,DISTANCES,DIST2,RIGIDBODY, &
  &                          RMAT,.FALSE.)
            DO J2=1,NATOMS
               WRITE(3,'(a4,2x,i5,2x,a4,a4,1x,i4,4x,3f8.3,2f6.2,6x,a4)') &
