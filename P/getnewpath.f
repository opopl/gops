...
C
     ...
     1                 LPOINTSTS(NR), LPLUS(NR), LMINUS(NR), LOCALPOINTS2(NR), DIST2
      DOUBLE PRECISION DUMMY
      DOUBLE PRECISION NEWFVIBMIN, NEWFVIBTS, NEWPOINTSMIN(NR), NEWPOINTSMINPREV(NR),
     1                 NEWPOINTSTS(NR), NEWIXMIN,  NEWIYMIN, NEWIZMIN,
...
!           ENDIF
         ...
         ENDIF
         NEWPOINTSMINPREV(1:NR)=NEWPOINTSMIN(1:NR)
C
...
C  
      IF (MINF.GT.0) THEN
            ...
            IF (ABS(NEWEMIN-EMIN(MINF)).LT.EDIFFTOL) THEN
               READ(UMIN,REC=MINF) (LOCALPOINTS2(J3),J3=1,NR)
               CALL MINPERMDIST(LOCALPOINTS,LOCALPOINTS2,NATOMS,DEBUG,BOXLX,BOXLY,BOXLZ,BULKT,TWOD,DISTANCE,DIST2,RIGIDBODY,
     &                          RMAT,.FALSE.)
            ENDIF
