...
USE COMMONS,ONLY : NPERMGROUP, NPERMSIZE, PERMGROUP, GEOMDIFFTOL

IMPLICIT NONE
INTEGER J1, NATOMS, NPMIN, NGMIN, J2, PERM(NATOMS), NDUMMY, NMATCHED, PATOMS, J3, J4, ND1
DOUBLE PRECISION DUMMYB(3*NATOMS),DUMMYA(3*NATOMS),DISTANCE,BOXLX,BOXLY,BOXLZ,XSHIFT,YSHIFT,ZSHIFT,XTEMP(3*NATOMS)
DOUBLE PRECISION DMIN, DTOTAL, DIST, GDSQ
...
PITEST=.FALSE.
SAMEMIN=.TRUE.
GDSQ=(GEOMDIFFTOL/NATOMS)**2 ! because GEOMDIFFTOL is usually used for the 3N dimensional space
NPMIN=HUGE(1)
...
outer: DO J1=ND1+1,ND1+NPMIN
   ...
   IF (.NOT.TWOD) ZSHIFT= &
  &       DUMMYA(3*(J2-1)+3)-DUMMYB(3*(ND1)+3)-BOXLZ*NINT((DUMMYA(3*(J2-1)+3)-DUMMYB(3*(ND1)+3))/BOXLZ)
   DO J2=1,NATOMS
