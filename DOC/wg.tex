...
\newcommand{\prog}{}
\newcommand{\keyw}[1]{\hyperlink{\prog:#1}{\it #1\/}}
\newcommand{\ksec}[2]{\subsubsection{#1\ {\it\ #2 \/}}\hypertarget{\prog:#1}}


\newcommand{\gmin}{{\tt GMIN}}
\newcommand{\optim}{{\tt OPTIM}}
\renewcommand{\ps}{{\tt PATHSAMPLE}}
\newcommand{\dps}{{\tt disconnectionDPS}}
\newcommand{\dman}{{\tt manipulate}}

\chapter{GMIN}
\renewcommand{\prog}{GMIN}
...
\ksec{AMCHNMAX}{\/}: The maximum number of angles that will be changed by up to {\it STEP\/} during an 
...
\ksec{ANGSTROM}{\/}: specifies coordinates in \AA ngstrom for the {\it FRAUSI\/}
potential.
...
\ksec{BSPTDUMPFRQ}{\/}: restart a previous {\it BSPT\/} or {\it PTMC\/} run.
...
\ksec{CHFREQ}{ nfreq}: used with {\it CHARMM} keyword to specify that every
{\it nfreq} basin-hopping steps dihedrals are twisted. Default is {\it nfreq}=1.

\ksec{CHNMAX}{}: used with {\it CHARMM} keyword to specify the maximum allowed
number of angles to be twisted. Specifies a dihedral angle step taking scheme. 

\ksec{CHNMIN}{}: used with {\it CHARMM} keyword to specify the minimum allowed
number of angles to be twisted.

\ksec{CHPMAX}{}: used with {\it CHARMM} keyword to specify the maximum allowed
probability for twisting an angle.

\ksec{CHPMIN}{}: used with {\it CHARMM} keyword to specify the minimum allowed
probability for twisting an angle.

\ksec{CHRIGIDROT}{ prot rotmax nrot}: used with {\it CHARMM} keyword 
...
\ksec{CHRIGIDTRANS}{ ptrans transmax ntrans}: used with {\it CHARMM} keyword 
...
\ksec{DIELEC}{ dparam\/}: specifies dielectric constant for {\it AMBER\/}.
...
\ksec{DONTMOVEGROUP}{ centre radius type\/}: If {\it type} is set as default to {\textrm GT}, {\it DONTMOVEGROUP\/} prevent all atom greater than {\it radius} angstroms from the {\it centre} atom from moving during MC step taking. {\it type} can also be set to LT to not move all atoms within {\it radius} of {\it centre}.
...
\ksec{DUMPQU}{\/}: when using {\it AMBER9\/}, dumps each quench geometry in rst format to quenchX.rst 
and pdb format to quenchX.pdb. Dumping does not occur if a chirality check fails.

\ksec{DUMPSTEPS}{\/}: when using {\it AMBER9\/}, dumps each geometry after the MC step has been taken in rst format to afterstepX.rst 
...
\ksec{FAKEWATER}{ \/}: specifies a distance-dependent dielectric in {\it AMBER\/}.
...
\ksec{FIXTEMP}{ \/}: explicitly fixes the temperature. Only used if {\it FIXSTEP\/} is set, in 
...
\ksec{FREEZEGROUP}{ centre radius type\/}: If {\it type} is set as default to {\textrm GT}, {\it FREEZEGROUP\/} FREEZEs all atom greater than {\it radius} angstroms from the {\it centre} atom. {\it type} can also be set to LT to FREEZE all atoms within {\it radius} of {\it centre}.
...
\ksec{GROUND}{\/}: when combined with keywords {\it NEON\/} or {\it ARGON\/}
...
\subsection{I-M}
\ksec{INTMIN}{}: used with {\it CHARMM} keyword to specify minimisation in internal 
...
\ksec{LIGMOVE}{ ligrotscale ligcartstep ligtransstep ligmovefreq\/}: used with {\it AMBER9\/} and {\it MOVABLEATOMS}. Specifies ligand only rotation, cartesian perturbation and translation. The ligand is defined by atom index in the file 'movableatoms'. Setting {\it ligrotscale} less than 1.0
...
\ksec{MULTIPLICITY}{ xmul\/}: specifies the multiplicity of the electronic state in {\it DFTB\/}
calculations.
...
\ksec{NOPHIPSI}{}: used with the {\it CHARMM} keyword to specify twisting of 
...
\ksec{PLUS}{\/}: when combined with keywords {\it NEON\/} or {\it ARGON\/}
uses a diatomics-in-molecules potential for the singly charged cation.

\ksec{PMAX}{ max\/}: {\it max\/} is the maximum probability of dihedral angle twisting for {\it AMBER\/}.

\ksec{PMIN}{ min\/}: {\it min\/} is the minimum probability of dihedral angle twisting for {\it AMBER\/}.
...
\ksec{QCUTOFF}{ qcut\/}: {\it qcut\/} is a distance cut-off for Coulomb interactions in {\it AMBER\/}.
...
\ksec{QUCENTRE}{\/}: sets the centre of coordinates to the origin (0,0,0) before each MC step is taken (so after each quench), but not during the minimisation itself unlike {\it CENTRE}. 
...
\ksec{RCUTOFF}{ rcut\/}: {\it rcut\/} is a distance cut-off in {\it AMBER\/}.
...
\ksec{RMS}{ rmslimit rmstol rmssave lca best}: used with {\it CHARMM} keyword to
...
\ksec{SETCENTRE}{ x y z\/}: Sets the centre of mass/coordinates (before the initial quench) to ({\it x,y,z\/}). For example, {\it SETCENTRE 0.0 0.0 0.0\/}
...
\ksec{SEED}{ nsstop\/}: if the {\it SEED\/} keyword appears then the program
...
\ksec{TOLBRENT}{ tolb\/}: parameter for {\it DBRENT\/} minimisation. 
Inefficient compared to LBFGS.

\ksec{TOMEGA}{}: used with the {\it CHARMM} keyword to specify that peptide bonds will be twisted along with all other dihedrals.
...
\ksec{TWOPLUS}{\/}: when combined with keywords {\it NEON\/} or {\it ARGON\/}
...
\ksec{UPDATES}{ nup\/}: {\it UPDATES\/} is the number of previous steps saved in the LBFGS routine,
default 4.
...
\ksec{ZETT}{1\/} and {\it ZETT2\/}: specify the Zetterling potentials.
% </kwd>
...
% \subsection {\it SAVECANDIDATES\/}: % unknown
...
\ksec{DIJINITCONTFLY}{ exp or n\/}: performs the same function as {\it DIJINITCONT} but
...
\ksec{DIJINITSTARTFLY}{ exp or n\/}: performs the same function as {\it DIJINITSTART} but
calculates distances between local minima on the fly. This approach may be necessary 
for initial path runs that produce more that 20,000 minima, where fitting all the 
distances into memory becomes an issue.
...
are run for {\it KMC} and {\it KMCcommit}. It also determines which state is the target
...
{\tt points.min} and write them to file {\tt extractmin}. If the {\it CHARMM}
...
\ksec{PERMISOMER}{\/}: equivalent to {\it PERMDIST\/} above, expect that
...
of {\it NATOMS\/} and {\it RIGIDBODIES} should be present.
...
{\it STARTFROMPATH a\/} from  file {\tt path.info.a} in the new {\it DUMPALLPATHS}
format.
...
\dps is a program which purpose is to plot disconnectivity graphs \cite{BeckerKarplus97}.


 It is adapted to read PATHSAMPLE.2.0 data files.
...
\ksec{DUMPSIZES}
 If present, a file called node\_sizes is written, listing how many minima
...
\ksec{IDENTIFY\_NODE}{max\_min}
 If present, the nodes are labelled with the format N1\_N2, where N1 is the number of level,
 N2 is the number of the node at that level. The label is only printed if the
 number of minima below that node is smaller than max\_min. With this info
 you can pick the number of minima corresponding to that node from the node\_numbers file,
...
Label $n$ different sections of the graph in colour as specified by the 
 ...
 be specified individually at both \keyw{COLOURMARKER} in this file. - vkd 
