      SUBROUTINE GBBOX (NATOMS, X, G, ENERGY, GTEST, STEST)

      USE MODHESS
      USE COMMONS, ONLY : PARAM1, PARAM2
      ...
      DOUBLE PRECISION :: SCSIG, SCSIG3, ENERGY, BOXL, GBSRC, RCUTSQ
      ...
      DOUBLE PRECISION :: SRCM1, SRCM2, SRCM6, SRCM7, SRCM12, SRCM13, SRC12M6
      ...
      BOXL   = PARAM1
      RCUTSQ = PARAM2*PARAM2
      GBSRC  = PARAM2 / GBSIGNOT
...
!     CALCULATE DEL(V)/DEL(R)
                  ...
                  SRCM1   = 1.D0/(GBSRC-SCSIG+1.D0)
                  SRCM2   = SRCM1*SRCM1
                  SRCM6   = SRCM2*SRCM2*SRCM2
                  SRCM7   = SRCM6*SRCM1
                  SRCM12  = SRCM6*SRCM6
                  SRCM13  = SRCM12*SRCM1
                  SRC12M6 = SRCM12 - SRCM6

                  FCT9   = 2.D0*SRM13 - SRM7
                  FCT21  = 2.D0*SRCM13 - SRCM7
                  VR     = -(24.D0/GBSIGNOT)*EPS*FCT9

!     CALCULATE ENERGY

                  ENERGY = ENERGY + EPS*(SR12M6 - SRC12M6)
...
!                  FCT10  = 24.D0*GBMU*GBCHIPRM*EPS/(GBSIGNOT*EPS2)
                  ...
                  VA     = -FCT11*(SR12M6-SRC12M6)*FCT7P8+FCT12*(FCT9-FCT21)*FCT3P4
                  VB     = -FCT11*(SR12M6-SRC12M6)*FCT7M8+FCT12*(FCT9-FCT21)*FCT3M4
...
!     CALCULATE DEL(V)/DEL(\GAMMA)
                  ...
                  FCT19  = FCT17*(SR12M6 - SRC12M6)
                  FCT20  = FCT18*(FCT9 - FCT21)
