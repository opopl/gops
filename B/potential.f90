...
10    CONTINUE
      ...
      ! IF...THEN {{{
      IF (MYBLNT) THEN 
         CALL EBLN(NATOMS,X,EA,GRAD,HESS,BLNTYPE,GRADT,SECT,LFH,.TRUE.)
         EREAL=EA(1)
      ELSE IF (P46) THEN
         CALL P46MERDIFF(LFH,.TRUE.,X,NATOMS,GRAD,EREAL,GRADT)
      ELSE IF (G46) THEN
         CALL G46MERDIFF(LFH,.TRUE.,X,NATOMS,GRAD,EREAL,GRADT)
      ELSE IF (BLNT) THEN
